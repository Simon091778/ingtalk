-- The anonymous board no longer exposes author gender. Gender remains a
-- private profile attribute used by non-board features.
...
returns table (
  id uuid, author_id uuid, nickname text, title text, body text, image_url text, created_at timestamptz,
...
language sql stable security definer set search_path = public as $$
  select
    post.id, post.author_id, post.anonymous_name, post.title, post.body, post.image_url, post.created_at,
...
returns table (
  id uuid, author_id uuid, nickname text, body text, created_at timestamptz,
  like_count bigint, liked_by_me boolean, parent_id uuid, parent_nickname text,
  reply_to_id uuid, reply_to_nickname text
)
language sql stable security definer set search_path = public as $$
  select
    comment.id, comment.author_id, comment.anonymous_name, comment.body, comment.created_at,
