...
-- Restore the identity-link timestamp that was previously rendered from the
-- generic identities array. The identifying value and timestamp always come
-- from the same current active mapping.
...
language sql stable security definer set search_path='' as $$
  with active_phone as (
    ...
    select count(*)::integer as candidate_count,min(phone) as phone,min(linked_at) as verified_at
    from active_phone
  )
  select jsonb_build_object(
    'status',case
      when resolved.candidate_count=1 then 'active'
      when resolved.candidate_count>1 then 'unavailable'
      when exists(
        ...
        select 1 from account_private.phone_identity_history history_row
        where history_row.account_id=target_account
      ) then 'unavailable'
      else 'none'
    end,
    'phone',case when resolved.candidate_count=1 then resolved.phone else null end,
    'verified_at',case when resolved.candidate_count=1 then resolved.verified_at else null end
  )
  from resolved;
$$;

create or replace function account_private.admin_account_google(target_account uuid) returns jsonb
language sql stable security definer set search_path='' as $$
  with active_google as (
    select google_identity.identity_data->>'email' as email,identity_row.linked_at
    from account_private.account_identities identity_row
  ...
  select jsonb_build_object(
    ...
    'verified_at',case
      when resolved.candidate_count=1 and resolved.email_count=1 then resolved.verified_at
      else null
    end
  )
  from resolved;
...
create function account_private.admin_account_kakao(target_account uuid) returns jsonb
language sql stable security definer set search_path='' as $$
  with active_kakao as (
    select identity_row.linked_at
    from account_private.account_identities identity_row
    where identity_row.account_id=target_account
      and identity_row.provider='kakao'
      and account_private.identity_active(identity_row)
  ), resolved as (
    select count(*)::integer as candidate_count,min(linked_at) as verified_at
    from active_kakao
  )
  select jsonb_build_object(
    'status',case
      ...
      when resolved.candidate_count>0 or exists(
        select 1 from account_private.account_identities identity_row
        where identity_row.account_id=target_account and identity_row.provider='kakao'
      ) then 'unavailable'
      else 'none'
    end,
    'verified_at',case when resolved.candidate_count=1 then resolved.verified_at else null end
...
revoke all on function account_private.admin_account_phone(uuid),
  account_private.admin_account_google(uuid),account_private.admin_account_kakao(uuid)
  from public,anon,authenticated;
grant execute on function account_private.admin_account_phone(uuid),
  account_private.admin_account_google(uuid),account_private.admin_account_kakao(uuid)
  to service_role;
...
begin
  ...
  select jsonb_build_object(
    ...
    'kakao',account_private.admin_account_kakao(p.id),
