...
-- Supabase Auth contains both canonical +82 and legacy Korean phone values
-- without the leading plus. Normalize only for comparison; keep the stored
-- value and its existing identity HMAC unchanged.
create function account_private.normalize_admin_phone(input_phone text) returns text
language sql immutable set search_path='' as $$
  select case
    when compact ~ '^010[0-9]{8}$' then '+82'||substr(compact,2)
    when compact ~ '^8210[0-9]{8}$' then '+'||compact
    when compact ~ '^\+8210[0-9]{8}$' then compact
    else null
  end
  from (select regexp_replace(trim(coalesce(input_phone,'')),'[[:space:]().-]','','g') compact) normalized;
$$;
revoke all on function account_private.normalize_admin_phone(text) from public,anon,authenticated;
grant execute on function account_private.normalize_admin_phone(text) to service_role;
...
begin
  ...
  normalized_phone:=account_private.normalize_admin_phone(normalized_search);
  ...
  with phone_matches as materialized (
    ...
    where normalized_phone is not null
      ...
      and identity_row.identity_hash in (
        account_private.hash_phone(normalized_phone),
        account_private.hash_phone(substr(normalized_phone,2)),
        account_private.hash_phone('0'||substr(normalized_phone,4))
      )
      and account_private.identity_active(identity_row)
      and current_phone.resolved->>'status'='active'
      and account_private.normalize_admin_phone(current_phone.resolved->>'phone')=normalized_phone
