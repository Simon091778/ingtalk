...
begin
  ...
  -- An active current Phone account is canonical. A directly verified old
  -- Social account contributes only its login methods, financial receipt
  -- routing, reward cooldowns and a higher point balance. Its profile/content
  -- are deliberately not imported into the current account.
  if other_id is not null and provider_name in ('google','kakao')
    ...
    if source_identity.account_id is null or not account_private.identity_active(source_identity)
      or not exists(select 1 from account_private.account_devices where id=binding_id and account_id=r.account_id)
      ...
      or exists(select 1 from account_private.account_identities b where b.account_id=other_id and b.provider not in ('google','kakao'))
      or (select count(*) from account_private.account_identities where account_id=other_id) not between 1 and 2
      or exists(select 1 from account_private.account_identities old_identity
        join account_private.account_identities current_identity on current_identity.account_id=r.account_id
          and current_identity.provider=old_identity.provider
        where old_identity.account_id=other_id)
      or exists(select 1 from public.profiles where id=other_id and status::text='suspended'
    ...
    insert into account_private.account_recovery_aliases(retired_account_id,canonical_account_id,reason)
      values(other_id,r.account_id,'verified_social_recovery') on conflict(retired_account_id) do nothing;
    if exists(select 1 from account_private.account_recovery_aliases a
        where a.retired_account_id=other_id and a.canonical_account_id<>r.account_id) then
      return jsonb_build_object('error','account_link_conflict'); end if;

    insert into public.point_reward_claims(user_id,reward_type,claimed_at)
      select r.account_id,c.reward_type,c.claimed_at from public.point_reward_claims c where c.user_id=other_id
    ...
    perform account_private.preserve_higher_point_balance(other_id,r.account_id);
    update public.point_purchase_receipts set user_id=r.account_id,updated_at=now()
      where user_id=other_id;
    perform public.delete_account_data(other_id);
    update account_private.account_identities set account_id=r.account_id where account_id=other_id;
    delete from account_private.device_accounts where id=other_id;
    ...
    return jsonb_build_object('ok',true,'account_id',r.account_id,'retired_social_account_id',other_id,
