...
-- Resolve an account's current phone from the authoritative Auth record without
-- copying raw phone data into application tables. This helper is private; the
-- public admin RPC below performs the caller authorization check.
create function account_private.admin_account_phone(target_account uuid) returns jsonb
...
revoke all on function account_private.admin_account_phone(uuid) from public,anon,authenticated;
grant execute on function account_private.admin_account_phone(uuid) to service_role;

-- Keep the existing one-request account detail flow. Only the minimum phone
-- result is added; Auth metadata, identity hashes and Auth user IDs stay private.
