begin;
create or replace function account_private.preserve_higher_point_balance(retired_account uuid,canonical_account uuid)
returns bigint language plpgsql security definer set search_path='' as $$
declare retired_balance bigint:=0; canonical_balance bigint:=0; preserved_balance bigint:=0;
begin
 if retired_account is null or canonical_account is null or retired_account=canonical_account then raise exception 'invalid_point_balance_merge'; end if;
 perform 1 from public.point_wallets where user_id in(retired_account,canonical_account) order by user_id for update;
 select coalesce((select balance from public.point_wallets where user_id=retired_account),0),
   coalesce((select balance from public.point_wallets where user_id=canonical_account),0)
   into retired_balance,canonical_balance;
 preserved_balance:=greatest(retired_balance,canonical_balance);
 if retired_balance>canonical_balance then
  insert into public.point_wallets(user_id,balance) values(canonical_account,preserved_balance)
    on conflict(user_id) do update set balance=greatest(public.point_wallets.balance,excluded.balance);
  insert into public.point_transactions(user_id,amount,reason,reference_id)
    values(canonical_account,retired_balance-canonical_balance,'account_merge_balance_preserved',retired_account)
    on conflict(user_id,reason,reference_id) do nothing;
 end if;
 return preserved_balance;
end; $$;
revoke all on function account_private.preserve_higher_point_balance(uuid,uuid) from public,anon,authenticated;
grant execute on function account_private.preserve_higher_point_balance(uuid,uuid) to service_role;
