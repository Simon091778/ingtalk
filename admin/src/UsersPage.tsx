...
type DetailTab = 'overview' | 'talks' | 'posts' | 'comments' | 'history' | 'points'
...
export function UsersPage({ role }: { role: AdminRole }) {
  ...
  const loadDetail = useCallback(async (userId: string) => {
    ...
    const { data, error: detailError } = await supabase.rpc('admin_get_user_detail', { target_user_uuid: userId })
    if (detailError) setError(detailError.message)
    else {
      const next = data as UserDetail
  ...
  return <>
    ...
    <section className="user-layout">
      <div className="user-list">{loading && users.length === 0 ? <div className="empty"><div className="spinner" />불러오는 중</div> : users.length === 0 ? <div className="empty">검색된 이용자가 없습니다</div> : users.map(user => <button key={user.user_id} className={`user-row ${selectedId === user.user_id ? 'selected' : ''}`} onClick={() => { setSelectedId(user.user_id); setTab('overview') }}><div className="user-avatar">{user.nickname[0]}</div><div className="user-row-main"><div><strong>{user.nickname}</strong><span className={`account-status ${user.status}`}>{user.status === 'active' ? '정상' : user.status === 'suspended' ? '정지' : user.status}</span></div><p>{user.user_id}</p><div className="user-metrics"><span>{Number(user.point_balance).toLocaleString()}P</span><span>톡 {user.talk_count}</span><span>글 {user.post_count}</span><span>댓글 {user.comment_count}</span>{user.report_count > 0 && <b>신고 {user.report_count}</b>}</div></div></button>)}</div>
      <div className="user-detail">{detailLoading && !detail ? <div className="empty"><div className="spinner" />이용자 정보 확인 중</div> : !detail ? <div className="detail-empty"><div className="empty-symbol">◎</div><h3>이용자를 선택해 주세요</h3><p>프로필, 콘텐츠, 신고와 제재 이력을 확인할 수 있습니다</p></div> : <>
        ...
        <div className="detail-tabs">{([['overview', '요약'], ['talks', `톡 ${detail.talks.length}`], ['posts', `게시글 ${detail.posts.length}`], ['comments', `댓글 ${detail.comments.length}`], ['history', `신고·제재 ${detail.reports.length + detail.actions.length}`], ['points', '포인트']] as const).map(([key, label]) => <button key={key} className={tab === key ? 'active' : ''} onClick={() => setTab(key)}>{label}</button>)}</div>
        <div className="user-tab-content">
          ...
          {tab === 'talks' && <ContentList empty="작성한 톡이 없습니다" items={detail.talks.map(item => ({ id: item.id, title: item.purpose, body: item.topic, meta: `${item.is_active ? '노출 중' : '비활성'} · ${date(item.created_at)}` }))} />}
