// Non-sign-in probe: starts PKCE authorization then cancels it. No Kakao account,
...
const appRedirect = 'ingtalk://auth/kakao'
...
authUrl.search = new URLSearchParams({ provider: 'kakao', redirect_to: appRedirect,
  code_challenge: challenge, code_challenge_method: 's256', prompt: 'login', scope: '' }).toString()
...
if (kakao.protocol !== 'https:' || kakao.hostname !== 'kauth.kakao.com') throw new Error('Unexpected OAuth provider destination')
const state = kakao.searchParams.get('state')
if (!state) throw new Error('Missing OAuth state')
const result = {
  project: new URL(base).hostname,
  oauthStartStatus: start.status,
  kakaoClientIdPresent: Boolean(kakao.searchParams.get('client_id')),
  kakaoClientIdFingerprint: createHash('sha256').update(kakao.searchParams.get('client_id') ?? '').digest('hex'),
  kakaoCallbackMatches: kakao.searchParams.get('redirect_uri') === expectedCallback,
  freshLoginRequested: kakao.searchParams.get('prompt') === 'login',
  minimalScopes: kakao.searchParams.get('scope') === '',
  requestedScopes: kakao.searchParams.get('scope'),
}
// An explicit denial exercises Supabase's configured return allowlist without
// obtaining a Kakao code or using the client secret to exchange any tokens.
...
result.kakaoDeveloperActivationTested = false
result.note = 'Supabase routing only. Verify Kakao Developers activation separately. No user signed in; Client Secret and device handoff need a real sign-in test.'
console.log(JSON.stringify(result, null, 2))
if (!result.minimalScopes || !result.kakaoClientIdPresent || !result.kakaoCallbackMatches || !result.appReturnMatches) process.exitCode = 1
