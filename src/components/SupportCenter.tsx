...
export function SupportCenterTrigger({ onPress }: { onPress: () => void }) {
  const i18n = useI18n()
...
export function SupportCenter({ controlledVisible, onVisibleChange, embeddedIos = false, showTrigger = true }: { controlledVisible?: boolean; onVisibleChange?: (visible: boolean) => void; embeddedIos?: boolean; showTrigger?: boolean }) {
  const i18n = useI18n()
  ...
  const keyboardInset = useIosKeyboardInset(embeddedIos && visible)
  ...
  const panel = <SwipeDismissView visible={visible} onDismissStart={() => setClosingBySwipe(true)} onDismiss={() => setVisible(false)}>
      <SafeAreaView edges={embeddedIos ? [] : keyboardVisible && Platform.OS === 'ios' ? ['top'] : ['top', 'bottom']} style={[styles.safe, embeddedIos && { paddingBottom: keyboardInset || insets.bottom }]}><KeyboardAvoidingView testID="support-keyboard-viewport" style={styles.safe} enabled={Platform.OS === 'android' || (!embeddedIos && Platform.OS === 'ios')} behavior={Platform.OS === 'android' || (!embeddedIos && Platform.OS === 'ios') ? 'padding' : undefined} keyboardVerticalOffset={0}>
        ...
        <View style={styles.composer}><TextInput value={body} onChangeText={setBody} multiline maxLength={2000} placeholder="문의 내용을 입력하세요" placeholderTextColor="#A8A29E" style={styles.input} /><Pressable disabled={!body.trim() || sending} onPress={() => void send()} style={[styles.sendButton, (!body.trim() || sending) && styles.disabled]}>{sending ? <ActivityIndicator color="#FFFFFF" /> : <Text style={styles.sendText}>전송</Text>}</Pressable></View>
      </KeyboardAvoidingView></SafeAreaView>
      </SwipeDismissView>
  return <>
    {showTrigger && <SupportCenterTrigger onPress={() => { setClosingBySwipe(false); setVisible(true) }} />}
    {embeddedIos ? visible && <View accessibilityViewIsModal style={styles.inlineOverlay}>{panel}</View> : <Modal visible={visible} animationType={Platform.OS === 'android' ? 'fade' : closingBySwipe ? 'none' : 'slide'} presentationStyle="fullScreen" onRequestClose={() => setVisible(false)}>{panel}</Modal>}
  </>
}

const styles = StyleSheet.create({
  inlineOverlay: { ...StyleSheet.absoluteFillObject, zIndex: 350, elevation: 350, backgroundColor: '#FFF9F5' }, card: { minHeight: 68, marginTop: 14, paddingHorizontal: 16, borderRadius: 16, borderWidth: 1, borderColor: '#E7DFDA', backgroundColor: '#FFFFFF', flexDirection: 'row', alignItems: 'center' }, cardText: { flex: 1, marginRight: 12 }, title: { color: '#292524', fontSize: 14, fontWeight: '900' }, description: { color: '#78716C', fontSize: 10, marginTop: 4 }, openButton: { minWidth: 72, minHeight: 38, borderRadius: 10, backgroundColor: '#F26B4B', alignItems: 'center', justifyContent: 'center' }, openButtonText: { color: '#FFFFFF', fontSize: 11, fontWeight: '900' },
