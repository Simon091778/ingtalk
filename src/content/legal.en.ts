...
8. Account deletion
Signup uses SMS, Google or Kakao verification without a user-set password or recovery code. After verifying the existing account and an additional sign-in method, linked phone, Google and Kakao sign-ins use one account, wallet and chat history on the registered phone. When you verify the current account and an additional sign-in method to link them and account ownership is confirmed, separately created accounts may be consolidated under the service's account-linking policy. Ordinary sign-in or matching phone numbers, emails or devices alone does not automatically consolidate separate accounts. Point balances are not added together; the higher of the two balances is retained. The retained account's profile, chats and posts are preserved, while the retired account's content is deleted without being transferred. Sign-in methods and records needed for payment audits, refunds and related purposes are handled under the account-linking policy and Privacy Policy. We do not provide manual merging of arbitrary separate accounts on support request. The same linked Google or Kakao account restores the existing account, points and chats across phones. Phone recovery requires a linked sign-in method and the registered device identity. Android uses an app-signing and OS-user-scoped device identifier; iPhone uses device-local Keychain data. Without a usable linked Google or Kakao account, losing access to linked sign-in methods, changing phones, a factory reset, signing or OS-user change, or loss of Keychain data may prevent restoration of previous points (including purchased points) and chats. Deletion applies to the entire account shared by linked methods and all its devices. Registration on all registered devices is restricted for 7 days after deletion; contact support if a secondhand device is affected. This technical restoration policy does not limit statutory payment or refund rights.
...
When you purchase points, RevenueCat and Apple App Store or Google Play process the internal app account ID, product, transaction and receipt identifiers, purchase time, amount, currency, country and refund information to verify purchases, credit points, prevent duplicate grants and handle refunds. Ingtalk does not directly collect card or bank account numbers. The internal app account ID recorded at purchase is retained separately as a payment-audit identifier without a phone number, email, or nickname, and remains after account linking or deletion for the period required for payment audits, refunds, legal obligations, and disputes.
